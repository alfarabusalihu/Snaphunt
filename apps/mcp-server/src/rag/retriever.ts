...
export async function queryDocuments(input: string, apiKey: string, options?: { topK?: number, requestId?: string, provider?: string, keywords?: string[] }) {
    let chunkResults;
    if (!input.trim()) {
        chunkResults = await getLatestChunks(options?.topK ?? 100); // Get more initially to filter
    } else {
        const queryEmbedding = await embedQuery(input, apiKey, options?.provider, options?.requestId);
        chunkResults = await searchVectors(queryEmbedding, options?.topK ?? 100);
    }

    // Keyword Gate Logic Removed: Relying on the LLM to filter by keywords
    // to prevent dropping valid candidates whose keywords were split across chunks.

    // Limit back to requested topK after filtering
    chunkResults = chunkResults.slice(0, options?.topK ?? 40);
