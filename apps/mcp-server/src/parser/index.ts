...
export async function parseInput(filepathOrUrl: string, apiKey?: string): Promise<ParsedDocument[]> {
    ...
    if (isUrl) {
        ...
        if (zipMatch) {
            ...
            if (stats.isDirectory()) return await parseDirectory(resolvedPath, apiKey);
    ...
    if (apiKey && text.trim().length > 0) {
        try {
            const google = (await import('@ai-sdk/google')).createGoogleGenerativeAI({ apiKey });
            const { generateText } = await import('ai');
            const model = google('gemini-2.5-flash');

            console.log(`🤖 Using Gemini to extract structured JSON for ${fileName}...`);
            const { text: jsonResponse } = await generateText({
                model,
                system: `You are an expert Resume Parser. Read the CV text below and extract ONLY the candidate's Skills, Work Experience, and Education. Return it strictly as a valid JSON object with this exact format (no markdown formatting like \`\`\`json, no other text):
{
  "skills": ["skill1", "skill2"],
  "experience": "Company A (2020-2023): Developer...",
  "education": "BSc Computer Science..."
}`,
                prompt: text.substring(0, 40000), // Protect against massive PDFs causing input overflow
                temperature: 0,
                maxOutputTokens: 2000
            });
            // Attempt to clean JSON
            const cleanedJson = jsonResponse.replace(/```json/g, '').replace(/```/g, '').trim();
            JSON.parse(cleanedJson); // Ensure it's valid JSON
            text = cleanedJson;
            console.log(`✅ Gemini Extraction successful for ${fileName}`);
        } catch (err: any) {
             console.error(`❌ Gemini Extraction failed for ${fileName}:`, err.message);
        }
    }
...
async function parseDirectory(dirPath: string, apiKey?: string): Promise<ParsedDocument[]> {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            const subResults = await parseDirectory(fullPath, apiKey);
            ...
            try {
                ...
                if (apiKey && text.trim().length > 0) {
                    try {
                        ...
                        console.log(`🤖 Using Gemini to extract structured JSON for ${entry.name}...`);
                        const { text: jsonResponse } = await generateText({
                            model,
                            system: `You are an expert Resume Parser. Read the CV text below and extract ONLY the candidate's Skills, Work Experience, and Education. Return it strictly as a valid JSON object with this exact format (no markdown formatting like \`\`\`json, no other text):
{
  ...
  "education": "BSc Computer Science..."
                        ...
                        text = cleanedJson;
                        console.log(`✅ Gemini Extraction successful for ${entry.name}`);
                    } catch (err: any) {
                         console.error(`❌ Gemini Extraction failed for ${entry.name}:`, err.message);
                    }
                }
                const checksum = calculateChecksum(buffer);
                results.push({
...
export async function scanInput(filepathOrUrl: string): Promise<any[]> {
    ...
    // Single file — return just that file, no sibling expansion
    const buf2 = await fs.readFile(resolvedPath);
    const checksum2 = calculateChecksum(buf2);
    console.log(`📄 Single file target: ${path.basename(resolvedPath)} (${(buf2.length / 1024).toFixed(1)} KB)`);
    return [{
        id: crypto.randomUUID(),
        fileName: path.basename(resolvedPath),
        location: resolvedPath,
        ...
        if (entry.isDirectory()) {
            ...
            if (isPdf) {
                try {
                    ...
                    console.log(`✅ Found PDF: ${entry.name} (${(buf.length / 1024).toFixed(1)} KB)`);
